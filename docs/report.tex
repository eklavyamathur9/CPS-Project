...
defines the key technical terms used throughout the report, and states the
problem in a concise form.
...
% ============================================================
\section{Technical Context of the Project}

This section explains \emph{why} the approach works, why it matters for
embedded and cyber-physical systems, and how the project is implemented.

\subsection{Underlying Principle}
\label{sec:principle}

\subsubsection{Why a Frequency Signature Can Identify a Key}
...
\subsubsection{Background: acoustic side-channel attacks}
...
\emph{fingerprints} learned from a known transcript.
...
This project distils that physical idea into a clean, tractable model. Rather
...
\subsubsection{The Matching Rule}
...
\subsection{Why This Matters for Embedded and Cyber-Physical Systems}
...
\subsubsection{Real-Time and Performance Constraints}
...
independent of the input length, and a lookup is $O(1)$. This demonstrates that
a resource-constrained device could hold the signature database on-chip with
minimal storage (e.g., a few hundred bytes of flash/ROM plus the code).

\subsubsection{Reliability and Safety Constraints}
...
\subsubsection{Summary of CPS Relevance}
...
This subsection describes the physical resources, the overall architecture,
and the software design of the simulator.
...
\subsubsection{Hardware and Platform Details}
...
\paragraph{Storage footprint.}
The complete signature database is a 48-entry constant table: stored as a
Python dict, it consumes on the order of a few kilobytes. Projecting this to an
embedded target, the (key, frequency) pairs fit in a few hundred bytes of
read-only memory, demonstrating that the approach is viable even for
memory-constrained devices. Runtime memory is proportional to the length of the
current input only, and all per-key processing uses a fixed, constant amount of
scratch space.

\subsubsection{Design Decisions and Alternatives Considered}
...
\subsubsection{Optimization Flow}

The project applied a deliberate sequence of optimizations. Each step is
summarised below and later quantified in the results section
(Figure~\ref{fig:opt}).

\begin{enumerate}
    \item \textbf{Constant lookup table:} key-to-frequency mapping is a static
          dict; frequencies are never recomputed, giving $O(1)$ access.
    \item \textbf{numpy-vectorised signal generation:} sine waves are built
          with vectorised array operations rather than per-sample Python
          loops, dramatically reducing plotting time.
    \item \textbf{Incremental live updaters:} the waveform plots are refreshed
          in place (\texttt{update\_}\allowbreak\texttt{spectrogram},
          \texttt{update\_sine\_plot})
          instead of being redrawn from scratch on every key press.
    \item \textbf{Plot cap:} \texttt{MAX\_PLOT\_KEYS} $=30$ bounds the number of
          rendered subplots so a paragraph with hundreds of keys cannot stall
          the GUI; analysis still covers the full sequence.
    \item \textbf{Robust WCET:} multi-trial median + P95 gives a trustworthy
          worst-case figure that is not skewed by a single slow measurement.
\end{enumerate}

\subsubsection{How to Run the Project}
...
\subsubsection{Results in tabular form}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lllll@{}}
        ...
        \bottomrule
    \end{tabular}
    \caption{Per-key analysis for the pangram (abridged to 35 of 45 keys;
...
\subsubsection{Example: punctuated paragraph reconstruction}
...
\subsubsection{Example: sample report output}
...
aspects, discusses the problems faced and bugs fixed, records the limitations,
and describes the optimization flow in detail.
...
\begin{itemize}
    \item \textbf{Correctness} is established by the deterministic separation
          proof (Section~\ref{sec:analysis}) and supported by the per-key
...
accurate---analysis for long paragraphs. Section~\ref{sec:analysis} revisits
...
\subsubsection{Attack surface and confidentiality}
...
\subsubsection{Security of the simulation itself}
...
The deterministic worst-case separation proof of Section~\ref{sec:analysis}
...
Liveness and termination are verified programmatically. The ranking function $V$
double as the correctness metric, and confidence $C$ translates error into an
